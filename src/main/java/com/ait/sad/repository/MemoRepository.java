...
public class MemoRepository {
	...
	public void save(Memo memo) {
		...
		try {
			...
			stat = conn
					.prepareStatement("insert into memo( name,topic, detail,  dateCreate,dateUpdate,dateRemind) values (?, ?, ?, ?,?,?)");
	...
	public Memo findOneById(int id) {
		...
		try {
			ps = conn
					.prepareStatement("select topic, detail,  dateCreate, dateUpdate from memo where id =?  limit 1");
			ps.setInt(1, id);
	...
	public ArrayList<Memo> findAll() {
		...
		try {
			ps = conn
					.prepareStatement("select id, topic, name, detail,  dateCreate, dateUpdate,dateRemind from memo ORDER BY dateRemind ASC;");
	...
	public void delete(int id) {
		...
		try {
			...
			stat = conn.prepareStatement("DELETE FROM MEMO WHERE ID = ?");
			stat.setInt(1, id);
